...
正如上面提到的，NSInvocationOperation 类是 NSoperation 抽象类的一个具体子类，当一个 NSInvocationOperation 开始执行时，它会调用我们指定的 object 的 selector 方法。通过使用 NSInvocationOperation 类，我们可以避免为每一个任务都创建一个自定义的子类，特别是当我们在修改一个已经存在的应用，并且这个应用中已经有了我们需要执行的任务所对应的 object 和 selector 时非常有用。
...
- (void)myTaskMethod1:(id)data {
    NSLog(@"start executing myTaskMethod1 with data: %@", data);
    sleep(3);
    NSLog(@"finish executing myTaskMethod1");
}
...
- (void)myTaskMethod2:(id)data {
    NSLog(@"start executing myTaskMethod2 with data: %@", data);
    sleep(3);
    NSLog(@"finish executing myTaskMethod2");
}
...
- (NSBlockOperation *)createBlockoperation {
    ...
    [blockoperation addExecutionBlock:^{
        NSLog(@"start executing block2");
        sleep(3);
        NSLog(@"finish executing block2");
    }];
    
    [blockoperation addExecutionBlock:^{
        NSLog(@"start executing block3");
        sleep(3);
...
我们需要用一个自定义的初始化方法来使 operation 在初始时处于一个已知的状态，并且重写 main 方法来执行我们的任务。当然，我们也可以实现一些其他额外的方法，比如实现 NSCoding 协议来允许我们归档和解档 operation 等。下面的代码展示了如何自定义一个简单的 operation ：
...
- (void)main {
    @try {
        NSLog(@"start executing main with data: %@", self.data);
        sleep(3);
        NSLog(@"finish executing main with data: %@", self.data);
    }
...
当一个 operation 开始执行后，它会一直执行它的任务直到任务执行完成或被明确取消为止。我们可以在任何时间点取消一个 operation ，甚至是在它还未开始执行之前。为了让我们自定义的 operation 能够支持取消事件，我们需要做的就是在代码中定期地检查 isCancelled 方法的返回值，一旦检查到这个方法返回 YES ，我们就需要立即停止执行接下来的任务。根据苹果官方的说法，isCancelled 方法本身是足够轻量的，所以就算是频繁地调用它也不会给系统带来太大的负担。
...
看到这里，我想你应该可以意识到一点，那就是尽管 operation 是支持取消操作的，但却并不是立即取消的，而是在你调用了 operation 的 cancel 方法后的下一个 isCancelled 的检查点取消的。
...
- isExecuting 和 isFinished，必须：并发执行的 operation 需要负责配置它们的执行环境，并且向外界客户报告执行环境的状态。因此，一个并发执行的 operation 必须要维护一些状态信息，用来记录它的任务是否正在执行，是否已经完成执行等。此外，当这两个方法所代表的值发生变化，我们需要生成相应的 KVO 通知，以便外界能够观察到这些状态的变化。
- isConcurrent，必须：这个方法的返回值用来标识一个 operation 是否是并发的 operation ，我们重写这个方法并返回 YES 即可。

下面将分成三个部分介绍一下定义一个并发的 operation 所需要的基本代码，主体部分的代码如下所示：
...
这部分代码看上去比较简单，但是却需要我们用心地去理解它。首先，我们用 @synthesize 关键字手动合成了两个实例变量 _executing 和 _finished ，然后分别在重写的 isExecuting 和 isFinished 方法中返回了这两个实例变量。另外，我们通过查看 NSoperation 类的头文件可以发现，executing 和 finished 属性都被声明成了只读的 readonly 。所以我们在 NSoperation 子类中就没有办法直接通过 setter 方法来自动触发 KVO 通知，这也是为什么我们需要在接下来的代码中手动触发 KVO 通知的原因。

接下来是 start 方法的代码，在这个方法中，我们最需要关注的点就是为 main 方法启动了一个新的线程，这是 operation 能够并发执行的关键所在。此外，在真正开始执行任务前，我们通过检查 isCancelled 方法的返回值来判断 operation 是否已经被 cancel ，如果 operation 已经被 cancel ，那么就直接返回了。
...
下面是真正执行任务的 main 方法，值得注意的是在任务执行完毕后，我们需要手动触动 isExecuting 和 isFinished 的 KVO 通知。

```
- (void)main {
    @try {
        NSLog(@"start executing main");
...
**注意**，有一个非常重要的点需要引起我们的注意，那就是即使一个 operation 是被 cancel 掉了，我们仍然需要手动触发 isFinished 的 KVO 通知。因为当一个 operation 依赖其他 operation 时，这个对象会观察所有其他对象的 isFinished 的值的变化，只有当它依赖的所有 operation 的 isFinished 的值为 YES 时，这个 operation 才能够开始执行。因此，如果当一个我们自定义的 operation 被取消掉但并没有手动触发 isFinished 的 KVO 通知的话，那么所有依赖它的 operation 都不会执行。

### 维护 KVO 通知

NSoperation 类的以下 key paths 支持 KVO 通知，我们可以通过观察这些 key paths 非常方便地监听到 operation 内部的状态变化。
...
通过配置依赖关系，我们可以让不同的 operation 串行执行，正如我们前面提到的，一个 operation 只有在它依赖的所有 operation 都执行完成后才能开始执行。配置 operation 依赖关系主要涉及到 NSoperation 类中的以下两个方法：
...
顾名思义，第一个方法用于添加依赖，第二个方法则用于移除依赖。需要特别注意的是，用 addDependency: 方法添加的依赖关系是单向的，比如 [A addDependency:B]; ，即 A 依赖 B，B 并不依赖 A 。 

另外，这里的依赖关系并不局限于相同 operation queue 中的 operation 之间。其实，从上面两个配置依赖关系的方法是存在于 NSoperation 类中的，我们也可以看出来，operation 的依赖关系是 operation 自己管理的，与它被添加到哪个 operation queue 无关。因此，我们完全可以给一些 operation 配置好依赖关系，然后将它们添加到不同的 operation queue 中。但是，有一个点是需要我们特别注意的，就是不要在 operation 之间添加循环依赖，这样会导致这些 operation 都不会被执行。
...
我们只能够在开始执行一个 operation 之前或者将其添加到 operation queue 前，通过 operation 的 setThreadPriority: 方法来配置它的线程优先级。当 operation 开始执行时，NSoperation 类中默认的 start 方法就会使用我们设置的值来修改当前线程的优先级。需要注意的是，我们指定的这个线程优先级只会影响 main 方法执行时所在线程的优先级。所有其它的代码，包括 operation 的 completion block 则一直会以默认的线程优先级执行。另外，在我们自定义一个并发的 operation 类时，我们也需要在 start 方法中自行配置线程的优先级。

### 设置 Completion Block

从 iOS 4.0 开始，一个 operation 可以在它的主任务完成时执行一个 completion block 。我们可以用 completion block 来执行一些主任务之外的任务，比如，我们可以用它来通知一些客户 operation 已经执行完毕，而并发的 operation 也可以用这个 block 来生成最终的 KVO 通知。如果需要设置一个 operation 的 completion block ，通过调用 NSoperation 类的 setCompletionBlock: 方法即可。
...
```
NSoperationueue* aueue = [[NSoperationueue alloc] init];
```
...
```
@implementation OQUseoperationueue
...
- (void)executeoperationsUsingoperationueue {
    ...
    [operationueue addoperations:@[ blockoperation1, blockoperation2 ] waitUntilFinished:NO];
    
    [operationueue addoperationWithBlock:^{
        NSLog(@"start executing block");
        sleep(3);
        NSLog(@"finish executing block");
    }];
    
    [operationueue waitUntilAlloperationsAreFinished];
}
...
**注意**，在将一个 operation 添加到 operation queue 后就不要再修改这个 operation 了。因为 operation 被添加到 operation queue 后随时可能会执行，这个是由系统决定的，所以再修改它的依赖或者所包含的数据就很可能会造成不良影响。
...
另外，我们应该一直通过 start 方法去手动执行一个 operation ，而不是 main 或其他的什么方法。因为默认的 start 方法在真正开始执行任务前为我们增加了很多安全性的检查，比如检查 operation 是否已取消等。另外，正如我们前面所说的，在默认的 start 方法中会生成一些必要的 KVO 通知，比如 isExcuting 和 isFinish ，而这些 KVO 的通知正是 operation 能够正确处理依赖关系的关键所在。

更进一步说，如果我们实现的是一个并发的 operation ，我们也应该考虑在启动 operation 前检查一下它的 isConcurrent 状态。如果它的 isConcurrent 状态为 NO ，那么我们就需要考虑一下是否在当前线程同步执行这个 operation ，或者是先为这个 operation 创建一个单独的线程，以供它异步执行。

当然，如果你已经能够确定一个 operation 的可执行状态，那么你大可不必做这些略显麻烦的防范性检查，直接调用 start 方法执行这个 operation 即可。下面的示例代码展示了手动执行一个 operation 的基本流程：
...
**注意**，我们应该要坚决避免在主线程中去同步等待一个 operation 的执行结果，阻塞的方式只应该用在辅助线程或其他 operation 中。因为阻塞主线程会让大大地降低我们应用的响应性，带来非常差的用户体验。
